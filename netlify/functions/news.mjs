...
function classifyContentType(feed, title) {
  ...
  if (
    /\breport\b|\bstudy\b|\bsurvey\b|\boutlook\b|\bwhite paper\b|\bdiscussion paper\b|\btoolkit\b|\bresearch\b|\breview\b|\bfindings\b|\bindex\b/.test(text)
  ) {
    return "report";
  }

  if (
    /\bcase study\b|\bguide\b|\bhow\b|\binsight\b|\bperspective\b|\binterview\b|\btrends\b|\btransformation\b|\bexplainer\b|\bwhy\b/.test(text)
  ) {
    return "insight";
  }

  if (
    /\blaunch\b|\brollout\b|\bintroduc|\bexpand|\bpartnership\b|\bapproval\b|\bmigration\b|\bstandard\b|\bimplementation\b|\bframework\b|\broadmap\b|\bpilot\b|\btest\b|\btrial\b|\bnew rule\b|\bupdate\b|\bupgrade\b|\bsettlement\b/.test(text)
  ) {
    return "update";
  }
  ...
  return itemBlocks
    .map((block) => {
      ...
      if (feed.linkMode === "indexed") {
        title = cleanIndexedTitle(title, feed);
      }
  ...
  try {
    const response = await fetch(feed.url, {
      headers: {
        ...
        "User-Agent":
          "Informado/3.0 (+https://informado.netlify.app/)"
      },
    ...
    const items = parseFeed(xml, feed).slice(
      0,
      MAX_ITEMS_PER_FEED
    );
...
function addCorroboration(items) {
  return items.map((item, index) => {
    ...
    for (let j = 0; j < items.length; j += 1) {
      ...
      if (other.feedName === item.feedName) {
        continue;
      }
...
function dedupeFilterAndSort(items) {
  ...
  const filtered = items
    ...
    .filter(item =>
      ageDays(item.pubDate) <= MAX_AGE_DAYS
    )
...
function sourceStatusFromResults(
  settled,
  finalItems
) {
  return SOURCES.map(source => {
    ...
    const errors = rejected
      .map(entry =>
        String(
          settled[entry.index]?.reason?.message ||
          settled[entry.index]?.reason ||
          "Unknown error"
        )
      )
      .filter(Boolean);
...
export default async () => {
  ...
  const items =
    dedupeFilterAndSort(rawItems);

  const sources =
    sourceStatusFromResults(
      settled,
      items
    );
